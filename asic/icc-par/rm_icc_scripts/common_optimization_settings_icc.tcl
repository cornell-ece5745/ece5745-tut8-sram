...
# ctorng: The set_power_options command was retired in the F-2011.09
# release. There is an entry in the error manual (PWR-808) that describes
# that we pretty much don't need to do anything. It says the functionality
# of the option is folded into the core power commands (e.g., place_opt)
# and to use the -power option there. The scripts already use the -power
# option, so we don't need to do anything.
#
# I've commented out the below four if statements. If you look at
# icc-par/rm_setup/icc_setup.tcl, you'll notice that only $LEAKAGE_POWER
# is set to TRUE. The place_opt command has the -power switch, so again,
